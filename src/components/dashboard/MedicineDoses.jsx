...
export const MedicineDoses = ({ onClose }) => {
    const initialMedicineState = {
        ...
        dispensed: false,
        lastDispensed: '',
        isExisting: false // Flag to track if it's an existing record
    };

    const [medicines, setMedicines] = useState(Array(4).fill().map(() => ({ ...initialMedicineState })));
    const [originalMedicines, setOriginalMedicines] = useState([]); // Store original data for comparison
    ...
    useEffect(() => {
        ...
        onValue(medicinesRef, (snapshot) => {
            if (snapshot.exists()) {
                ...
                setOriginalMedicines([...medicineArray]); // Save original data
                
                // Mark existing medicines with the isExisting flag
                const filledMedicines = medicineArray.map(med => ({
                    ...med,
                    isExisting: true,
                    // Ensure fromDate and toDate are properly formatted
                    fromDate: med.fromDate ? new Date(med.fromDate) : null,
                    toDate: med.toDate ? new Date(med.toDate) : null
                }));
                
                // Fill remaining slots with empty medicine states
                const updatedMedicines = [
                    ...filledMedicines,
                    ...Array(4 - filledMedicines.length).fill().map(() => ({ ...initialMedicineState }))
                ].slice(0, 4);
    ...
    // Check if medicine has been modified compared to original
    const hasChanged = (medicine) => {
        if (!medicine.isExisting) return true; // New medicine always needs saving
        
        const original = originalMedicines.find(m => m.id === medicine.id);
        if (!original) return true;
        
        // Compare relevant fields
        return (
            medicine.name !== original.name ||
            parseInt(medicine.pills, 10) !== original.pills ||
            medicine.hour !== original.hour ||
            medicine.minute !== original.minute ||
            medicine.conditions !== original.conditions ||
            (medicine.fromDate && new Date(medicine.fromDate).toISOString() !== original.fromDate) ||
            (medicine.toDate && new Date(medicine.toDate).toISOString() !== original.toDate)
        );
    };
    ...
    const handleSubmit = async () => {
        try {
            // Validate and filter medicines that need to be saved
            ...
            for (const medicine of filledMedicines) {
                // Skip if the medicine is existing and hasn't changed
                if (medicine.isExisting && !hasChanged(medicine)) {
                    continue;
                }
                
                changesCount++;
                const medicineId = medicine.isExisting ? medicine.id : generateUniqueId();
                const medicineData = {
                    id: medicineId,
                    name: medicine.name,
                    ...
                    dispensed: medicine.isExisting ? medicine.dispensed : false,
                    lastDispensed: medicine.isExisting ? medicine.lastDispensed : '',
            ...
            if (changesCount === 0) {
                ...
                alert(`${changesCount} medicine(s) saved successfully!`);
            }
